...
def generate_transactions_PDF(user_id: str, start_date: str, end_date: str, account_number: str = None) -> None:
    try:
        ...
        send_attachment_mail(user, start_date, end_date, pdf)
...
def send_attachment_mail(user, start_date, end_date, pdf) -> None:
    subject = _('Your translations history PDF')
    message = (
        f'Dear {user.full_name},',
        'We hope this email finds you well.',
        'We have attached your recent transaction history as a PDF file.',
        'Please review and let us know if you need any further assistance.',
        f'Best regards,',
        f'The {settings.SITE_NAME} Team'
    )
    from_email = settings.DEFAULT_FROM_EMAIL
    recipient_list = [user.email]
    email = EmailMessage(subject, '\n'.join(message), from_email, recipient_list)
